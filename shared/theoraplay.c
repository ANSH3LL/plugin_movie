...
// OpenAL really likes its pcm samples as shorts instead of floats, so here we go.
// There are more modifications to this end later in the file.
    ...
    while (!ctx->halt && !eos)
    {
        ...
        while (!ctx->halt && vpackets)
        {
            ...
            if (frames > 0)
            {
                ...
                item->frames = frames;
                item->samples = (short *) malloc(sizeof (short) * frames * channels);
                item->next = NULL;
                ...
                for (frameidx = 0; frameidx < frames; frameidx++)
                {
                    for (chanidx = 0; chanidx < channels; chanidx++)
                        *(samples++) = float2short(pcm[chanidx][frameidx]);
                } // for
