 ...
 *
 * This file written by Ryan C. Gordon and slightly modified by ANSH3LL.
 */
...
static unsigned char *THEORAPLAY_CVT_FNNAME_420(const th_info *tinfo, const th_ycbcr_buffer ycbcr)
{
    ...
    const float yexcursionI = 0.0045662100456621f; // yexcursion = 219.0f;
    const float cboffset = 128.0f;
    const float cbexcursionI = 0.0044642857142857f; // cbexcursion = 224.0f;
    const float croffset = 128.0f;
    const float crexcursionI = 0.0044642857142857f; // crexcursion = 224.0f;
    ...
    if (pixels)
    {
        ...
        const int cboff = (tinfo->pic_x * 0.5) + (cbstride) * (tinfo->pic_y * 0.5);
        ...
        for (posy = 0; posy < h; posy++)
        {
            ...
            {
                const float y1 = (((float) py[posx]) - yoffset) * yexcursionI;
                const float y2 = (((float) py[posx + 1]) - yoffset) * yexcursionI;
                const float pb = (((float) pcb[poshalfx]) - cboffset) * cbexcursionI;
                const float pr = (((float) pcr[poshalfx]) - croffset) * crexcursionI;
